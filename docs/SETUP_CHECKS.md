...
本リポジトリのエージェント向け指示は [`AGENTS.md`](../AGENTS.md) に集約されています。
AIエージェントは、本リポジトリが `repo-starter-kit` テンプレートから生成された直後と
判断できる場合（プレースホルダが残っている、または `make init` が未実行の場合）、
会話の冒頭で以下をチェックし、未対応の項目をユーザーに通知・対応を促してください。
